...
Returns the navigator object. This currently has four properties, 
C<appName>
(set to C<ref $w>), C<appCodeName> (the same), C<appVersion> 
(C<< ref($w)->VERSION >>) and 
C<userAgent> (same as C<< $w->agent >>).

You can pass values to C<appName> and C<appVersion> to set them.

It also has a C<javaEnabled> method that always returns false. (If anyone
writes a Java plugin, please let me know, and I'll provide a way to make it
return true.)
...
This schedules the code to run after C<$ms> seconds have elapsed, 
returning a
...
This returns the window's named, if applicable. For a frame, this comes
